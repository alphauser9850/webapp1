...
<<<<<<< HEAD
def user_dashboard():
    # Get only categories that have active servers
    categories = Category.query.join(Server).filter(
...
@login_required
def category_servers(id):
    category = Category.query.get_or_404(id)
    
    # Get only active servers that are assigned to the user
    servers = Server.query.filter(
        Server.category_id == id,
        Server.is_active == True,
        Server.assigned_users.any(id=current_user.id)
    ).all()
    
    return render_template('main/category_servers.html',
                         category=category,
                         servers=servers)

@main.route('/server/<int:id>/start')
@login_required
...
<<<<<<< HEAD
        ...
        return redirect(url_for('main.lab_session', id=active_session.id))
    ...
    session = Session(
        ...
        is_active=True,
        hours_allocated=current_user.remaining_hours  # Use all available hours
    )
    
    try:
        session.save()
        return redirect(url_for('main.lab_session', id=session.id))
    except Exception as e:
        flash('Failed to start session.', 'error')
        return redirect(url_for('main.user_dashboard'))

@main.route('/session/<int:id>')
@login_required
def lab_session(id):
    ...
    # If session has ended, redirect to dashboard
    if session.end_time:
        flash('This session has ended', 'info')
        return redirect(url_for('main.user_dashboard'))
    
    # Check if server is still active
    if not session.server.is_active:
        session.end()
        db.session.commit()
        flash('Server is no longer active', 'error')
        return redirect(url_for('main.user_dashboard'))
    
    # Calculate remaining time
    remaining_hours = session.remaining_time
    ...
    if remaining_hours <= 0:
        ...
        return render_template('labs/session.html',
                            session=None,
                            has_time=False,
                            error_message="Session time has expired.")
    
    # Use the correct VNC password
    vnc_password = "Crysis7972@@!!"
...
@main.route('/session/<int:id>/end', methods=['POST'])
@login_required
def end_session(id):
    session = Session.query.get_or_404(id)
    
    if session.user_id != current_user.id and not current_user.is_admin:
        flash('You do not have permission to end this session.', 'danger')
        return jsonify({'error': 'Permission denied'}), 403
    
    if not session.end_time:
        try:
            # End the session (this will also update user's remaining hours)
            session.end()
            db.session.commit()
