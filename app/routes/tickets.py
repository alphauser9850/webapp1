...
<<<<<<< HEAD
    if current_user.is_admin:
        ...
        resolved_count = Ticket.query.filter_by(status='closed').count()
    else:
        ...
        in_progress_count = Ticket.query.filter_by(user_id=current_user.id, status='in_progress').count()
        resolved_count = Ticket.query.filter_by(user_id=current_user.id, status='closed').count()
...
def create_ticket():
    ...
    if form.validate_on_submit():
        ...
        notify_admins('new_ticket', ticket_data)

        flash('Your ticket has been created successfully.', 'success')
...
<<<<<<< HEAD
    ...
    if current_user.is_admin and not ticket.admin_viewed:
...
def clear_history():
<<<<<<< HEAD
    """Clear all closed tickets from the system."""
    if not current_user.is_admin:
        flash('You do not have permission to perform this action.', 'error')
        return redirect(url_for('tickets.dashboard'))
    
    try:
        # Delete all closed tickets and their messages
        ...
        # Delete the tickets
        Ticket.query.filter_by(status='closed').delete()
        db.session.commit()
        
        flash('Ticket history has been cleared successfully.', 'success')
    except Exception as e:
        db.session.rollback()
        flash('An error occurred while clearing ticket history.', 'error')
        print(f"Error clearing ticket history: {str(e)}")
